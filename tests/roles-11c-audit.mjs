...
assert(accessSource.includes("version: '11A'"),'AccessControl is not 11A');
...
assert(registrySource.includes("version: '11B'"),'ModuleRegistry is not 11B');
...
assert(apiSource.includes("version:'11C'"),'Roles facade is not 11C');
assert(loaderSource.includes('roles-api.js?v=20260919-roles11c'),'Roles facade loader marker missing');
...
await page.waitForFunction(()=>window.DiagnostikaRoles?.version==='11C'
  && window.DiagnostikaPlatform?.access?.version==='11A'
  && window.DiagnostikaPlatform?.modules?.version==='11B',
  null,{timeout:15000});
...
const initial=await page.evaluate(ids=>{
  ...
  for(const type of ['access:role-changed','module:access-denied','module:access-reconciled','module:stopped','module:started']){
...
await page.evaluate(()=>window.DiagnostikaRoles.set('specialist',{source:'roles-audit-specialist'}));
await page.waitForFunction(ids=>ids.every(id=>window.DiagnostikaPlatform.modules.get(id)?.status==='started'),moduleIds,{timeout:10000});
...
console.log('ROLES_11C_SUCCESS',JSON.stringify({
  roleTransitions:['specialist','client','admin','specialist'],
