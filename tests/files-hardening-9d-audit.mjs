...
const filesLoaderMatch=indexSource.match(/app-loader\.js\?v=([^"&]+)/);
assert(filesBuildMatch,'Files global build marker missing');
assert(filesLoaderMatch,'Files app-loader cache marker missing');
assert.equal(filesBuildMatch[1],filesLoaderMatch[1],'Files global build/app-loader markers differ');
