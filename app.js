...
function renderClient(){const c=client();if(!c){requestId=null;situationId=null;selected=null;renderRequests();renderMode();return;}if(!requestId||!c.requests.some(r=>r.id===requestId))requestId=c.requests[0]?.id||null;renderRequests();renderSessions();renderMode()}
...
function renderMode(){const diag=mode==='diagnosis';$('#diagnosisWorkspace').hidden=!diag;document.querySelector('.home-dashboard')?.toggleAttribute('hidden',diag);$('#centerPanel').classList.toggle('hidden',!diag);$('#rightPanel').classList.toggle('hidden',!diag);$('#diagnosticsLeft').classList.toggle('hidden',!diag);if(diag)renderTree()}
...
$('#addSituationBtn').onclick=()=>{const c=client(),r=request(),api=window.DiagnostikaDiagnosis;if(!c||!r||!api?.moduleAware)return;const s=api.addSituation({}, {client:c,requestId:r.id,source:'diagnosis-ui-situation-add',render:false});if(!s)return;situationId=s.id;selected=null;renderSituationList()};
$('#editSituationBtn').onclick=()=>{const c=client(),r=request(),s=situation(),api=window.DiagnostikaDiagnosis;if(!c||!r||!s||!api?.moduleAware)return;const changes={};const n=prompt('Название ситуации',s.name||'');if(n!==null)changes.name=n;const l=prompt('Дискомфорт 1–10',s.level);if(l!==null)changes.level=lvl(l);if(!Object.keys(changes).length)return;if(!api.updateSituation(s.id,changes,{client:c,requestId:r.id,source:'diagnosis-ui-situation-edit',render:false}))return;selected=null;renderSituationList()};
$('#deleteSituationBtn').onclick=()=>{const c=client(),r=request(),s=situation(),api=window.DiagnostikaDiagnosis;if(!c||!r||!s||!api?.moduleAware)return;if(confirm('Удалить ситуацию?')){if(!api.removeSituation(s.id,{client:c,requestId:r.id,source:'diagnosis-ui-situation-delete',render:false}))return;const live=request();situationId=live?.situations?.[0]?.id||null;selected=null;renderSituationList()}};
