...
(() => {
  ...
  function renderPaymentList(c,r){
    ...
    [...p.payments].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).forEach(pay=>{
      ...
      row.querySelector('.pr-save').onclick=()=>{
        ...
        save();renderSummary(c,r);refreshPaymentBox();refreshDatabasePaymentColumn();renderAllPayments();window.DiagnostikaHomeDashboard?.refresh?.();
      };
      row.querySelector('.pr-delete').onclick=()=>{p.payments=p.payments.filter(x=>x.id!==pay.id);save();renderPaymentList(c,r);renderSummary(c,r);refreshPaymentBox();refreshDatabasePaymentColumn();renderAllPayments();window.DiagnostikaHomeDashboard?.refresh?.();};
  ...
  q('#paymentMode').onchange=e=>{const c=currentClient(),r=currentRequest(c);if(!c||!r)return;paymentOfRequest(c,r).mode=e.target.value;save();renderDialog();refreshPaymentBox();refreshDatabasePaymentColumn();window.DiagnostikaHomeDashboard?.refresh?.();};
  q('#paymentCurrency').onchange=e=>{const c=currentClient(),r=currentRequest(c);if(!c||!r)return;const p=paymentOfRequest(c,r);p.currency=e.target.value;p.currencyManual=true;c.currency=e.target.value;c.currencyManual=true;save();renderSummary(c,r);renderPaymentList(c,r);renderAllPayments();refreshPaymentBox();window.DiagnostikaHomeDashboard?.refresh?.();};
  q('#paymentTotal').oninput=e=>{const c=currentClient(),r=currentRequest(c);if(!c||!r)return;paymentOfRequest(c,r).total=num(e.target.value);save();renderSummary(c,r);refreshPaymentBox();refreshDatabasePaymentColumn();window.DiagnostikaHomeDashboard?.refresh?.();};
  q('#paymentAddBtn').onclick=()=>{const c=currentClient(),r=currentRequest(c);if(!c||!r)return;const p=paymentOfRequest(c,r),amount=num(q('#paymentAmount').value);if(amount<=0){q('#paymentAmount').focus();return;}p.payments.push({id:uidPay(),date:q('#paymentDate').value||todayLocal(),amount,note:q('#paymentNote').value.trim(),receiptUrl:q('#paymentReceipt').value.trim()});save();q('#paymentAmount').value='';q('#paymentNote').value='';q('#paymentReceipt').value='';renderPaymentList(c,r);renderSummary(c,r);refreshPaymentBox();refreshDatabasePaymentColumn();window.DiagnostikaHomeDashboard?.refresh?.();};
  ...
  function refreshPaymentBox(){ensurePaymentBox();const c=currentClient(),r=currentRequest(c),box=document.querySelector('#clientPaymentBox');if(!box||!c)return;if(!r){box.querySelector('.client-payment-title').textContent='ОПЛАТА';box.querySelector('.client-payment-summary').textContent='Нет текущего запроса';return;}const s=summary(c,r),n=requestNumber(c,r),sym=symbolFor(c,r);box.querySelector('.client-payment-title').textContent=`ОПЛАТА · ЗАПРОС ${n}`;let text=s.label;if(s.count)text+=` · ${s.count} сессий`;else if(s.total||s.paid)text+=` · ${money(s.paid)} / ${money(s.total)} ${sym}`;box.querySelector('.client-payment-summary').textContent=text;}
  ...
  function refresh(){refreshPaymentBox();refreshDatabasePaymentColumn();if(dlg.open){const c=currentClient(),r=currentRequest(c);if(c&&r){renderSummary(c,r);renderPaymentList(c,r);}}if(allDlg.open)renderAllPayments();}
